...
if [ ! -f "config.json" ]; then
    echo "config.json이 없습니다. configs/khmer/config.json을 복사합니다..."
...
if [ -f "$PID_FILE" ]; then
    ...
    if ps -p "$OLD_PID" > /dev/null 2>&1; then
        ...
        echo "중지하려면: kill $OLD_PID 또는 ./scripts/stop-khmer.sh"
        exit 1
    else
...
# 실행
echo "크메르어 번역 봇을 시작합니다..."
node src/index.js &
NEW_PID=$!
echo $NEW_PID > "$PID_FILE"
echo "✅ 크메르어 번역 봇이 시작되었습니다 (PID: $NEW_PID)"
