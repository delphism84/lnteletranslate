...
한국어 ↔ 베트남어 자동 번역 텔레그램 봇 설정 가이드입니다.

## 설정 파일

`config.json` 파일을 편집하여 다음 정보를 입력하세요:

- `telegramBotToken`: 베트남어 번역 봇의 텔레그램 봇 토큰
...
서비스 이름을 `lnteletranslate-vietnam`로 설정하거나 스크립트를 수정하여 사용하세요.
...
**참고**: 베트남어 봇은 포트 58011을 사용합니다 (크메르어 봇과 구분).
...
2. `koreanTo`와 `vietnameseTo` 설정 확인
3. OpenAI API 할당량 확인
